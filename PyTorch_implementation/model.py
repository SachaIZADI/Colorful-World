...
class Generator(nn.Module):
    ...
    def __init__(self):
        ...
        self.decod8 = nn.Sequential(
            nn.ConvTranspose2d(in_channels=512, out_channels=512, kernel_size=3, padding=1, stride=2),
            #TODO: check this
            nn.ReLU(),
            #nn.Dropout2d(0.2),
            nn.BatchNorm2d(512),
        ...
        self.decod7 = nn.Sequential(
            nn.ConvTranspose2d(in_channels=2*512, out_channels=512, kernel_size=3, padding=1, stride=2),
            # TODO: check this
            nn.ReLU(),
            # nn.Dropout2d(0.2),
            nn.BatchNorm2d(512),
        ...
        self.decod4 = nn.Sequential(
            nn.ConvTranspose2d(in_channels=2*512, out_channels=256, kernel_size=3, padding=1, stride=2),
            # TODO: check this
            nn.ReLU(),
            # nn.Dropout2d(0.2),
            nn.BatchNorm2d(256),
        ...
        self.decod3 = nn.Sequential(
            nn.ConvTranspose2d(in_channels=2*256, out_channels=128, kernel_size=3, padding=1, stride=2),
            # TODO: check this
            nn.ReLU(),
            # nn.Dropout2d(0.2),
            nn.BatchNorm2d(128),
        ...
        self.decod2 = nn.Sequential(
            nn.ConvTranspose2d(in_channels=2*128, out_channels=64, kernel_size=3, padding=1, stride=2),
            # TODO: check this
            nn.ReLU(),
            # nn.Dropout2d(0.2),
            nn.BatchNorm2d(64),
        ...
        self.decodout = nn.Sequential(
            # TODO: check this
            nn.ConvTranspose2d(in_channels=2*64, out_channels=3, kernel_size=3, padding=1, stride=2),
            nn.Tanh())
    ...
    def forward(self, x):
        ...
        # --------- Decoder ---------
        d1 = self.decod1(e4)
        d2 = self.decod2(torch.cat([d1, e4], 1)) # concatenating layers cf. U-net
        d3 = self.decod3(torch.cat([d2, e3], 1)) # concatenating layers cf. U-net
        d4 = self.decod4(torch.cat([d3, e2], 1)) # concatenating layers cf. U-net
        d5 = self.decod5(torch.cat([d4, e1], 1)) # concatenating layers cf. U-net

        out = self.decodout(d5)
...
class GAN(object):
    ...
    def data_init(self):
        ...
        self.training_data_loader = DataLoader(dataset=self.training_dataset, batch_size=self.config.batch_size, shuffle=True)
